...
# Dashboard sembolü -> veri sağlayıcı sembolü
TICKERS = {
    "CVKMD": "CVKMD",
    "ALTINS1": "ALTIN",
    "ASTOR": "ASTOR",
...
def indicators(df):
    closes = [
        num(x) for x in df["Close"].tolist()
    ...
    return {
        ...
        "volume_vs_20d": (
            vol_now / avg_vol
            if vol_now is not None and avg_vol else None
        ),
    }
...
def fetch(display_symbol, provider_symbol):
    ...
    if df is None or len(df) < 30:
        raise RuntimeError(
            f"{display_symbol}: yeterli tarihsel veri alınamadı"
        )
    ...
    return {
        ...
        "quote": {
            ...
            "volume": ind["volume"],
            "market_cap": num(
                fast_info_value(info, "market_cap")
            ),
...
def main():
    ...
    for display_symbol, provider_symbol in TICKERS.items():
        try:
            result["stocks"][display_symbol] = fetch(
                display_symbol,
                provider_symbol
            )
            ...
            print(
                f"ERROR: {display_symbol} <- "
                f"{provider_symbol}: {exc}"
            )

    if not result["stocks"]:
        raise RuntimeError(
            f"Hiçbir hisse verisi alınamadı: {result['errors']}"
        )
